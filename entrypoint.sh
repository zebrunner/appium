...
$CMD &
java ${JAVA_OPTS} -Djava.security.egd=file:/dev/./urandom -XX:InitialRAMPercentage=25.0 -cp /opt/mcloud-node-1.0.jar:/opt/mcloud-node.jar -javaagent:/opt/mcloud-node-agent.jar org.openqa.selenium.grid.Bootstrap node \
  --port 7777 \
