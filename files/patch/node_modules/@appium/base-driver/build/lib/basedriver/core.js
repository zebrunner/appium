...
class DriverCore extends extension_core_1.ExtensionCore {
    ...
    async getStatusWDA(exitCode) {
        ...
        if (!wdaURL) {
            throw new Error('[MCLOUD] Environment variable WDA_ENV is undefined');
        }
        const status = await (0, mcloud_utils_1.getWDAStatus)(wdaURL);
        if (!status || 'success' !== status.value.state) {
            if (exitCode != null) {
                this.log.error('[MCLOUD] Error for sending of WDA status http call. See appium logs for details');
                this.log.warn(`[MCLOUD] Killing Appium instance with "exit(${exitCode})"`);
                process.exit(exitCode);
            }
            throw new Error('[MCLOUD] Error for sending of WDA status http call. See appium logs for details');
        }
    ...
    async getStatusADB(exitCode) {
        ...
        if (deviceUDID) {
            ...
            try {
                ...
                if (exitCode != null) {
                    this.log.error(`[MCLOUD] Connected device with UDID ${deviceUDID} is NOT ready for execution. Device was not returned by adb`);
                    this.log.warn(`[MCLOUD] Killing Appium instance with "exit(${exitCode})"`);
                    process.exit(exitCode);
                }
                throw new Error(`[MCLOUD] Connected device with UDID ${deviceUDID} is NOT ready for execution. Device was not returned by adb`);
            }
        }
        else {
            const deviceName = process.env.ANDROID_DEVICES;
            if (!deviceName) {
                throw new Error(`[MCLOUD] Neither DEVICE_UDID nor ANDROID_DEVICES environment variables were found.`);
            }
            ...
            try {
                ...
                throw new Error(`[MCLOUD] Connected device with name ${deviceUDID} is NOT ready for execution. Device was not returned by adb`);
            }
        }
    }
    /**
