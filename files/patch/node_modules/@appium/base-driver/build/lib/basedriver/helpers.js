...
const APPLICATIONS_CACHE = new lru_cache_1.LRUCache({
    ...
    dispose: ({ fullPath }, app) => {
        ...
        if (fullPath) {
            logger_1.default.info(`[MCLOUD] APPLICATIONS_CACHE setting. Deleting files by path: ${fullPath}`);
...
process.on('exit', () => {
    ...
    for (const appPath of appPaths) {
        try {
            logger_1.default.info(`[MCLOUD] process.on('exit'). Deleting files by path: ${appPath}`);
    ...
    return await APPLICATIONS_CACHE_GUARD.acquire(appCacheKey, async () => {
        ...
        if (isUrl) {
            ...
            try {
                ...
                localAppsFolder = await (0, mcloud_utils_1.getLocalAppsFolder)();
                let localFile;
                let fileName;
                let lockFile = '';
                ...
                if (localAppsFolder !== undefined) {
                    ...
                    if (await support_1.fs.exists(localFile)) {
                        if (appSizeCheckDisabled === false) {
                            ...
                            logger_1.default.info(`[MCLOUD] Local version of app was found. Will check actuality of the file`);
                            // At this point local file might be deleted by parallel session which updates outdated app
                            let attemptsCount = 0;
                            while (!await support_1.fs.exists(localFile) && (attemptsCount++ < maxAttemptsCount)) {
                                await new Promise((resolve) => {
                                    logger_1.default.info(`[MCLOUD] Attempt #${attemptsCount} for local app file to appear again`);
                            ...
                            const stats = await support_1.fs.stat(localFile);
                            const localFileLength = stats.size;
                            const remoteFileLength = await (0, mcloud_utils_1.getFileContentLength)(app);
                            logger_1.default.info(`[MCLOUD] Remote file size is ${remoteFileLength} and local file size is ${localFileLength}`);
                            if (remoteFileLength !== localFileLength) {
                                logger_1.default.info(`[MCLOUD] Sizes differ. Hence that's needed to download fresh version of the app`);
                                if (await support_1.fs.exists(localFile)) {
                                    await support_1.fs.unlink(localFile);
                                }
                                else {
                                    logger_1.default.warn(`[MCLOUD] Old local application file ${localFile} was not found. Probably it was removed by another thread which was downloading app in parallel`);
                                }
                                downloadIsNeeded = true;
                            }
                            else {
                                logger_1.default.info(`[MCLOUD] Sizes are the same. Hence will use already stored application for the session`);
                                newApp = localFile;
                                downloadIsNeeded = false;
                            }
                        }
                        else {
                            logger_1.default.info(`[MCLOUD] APPIUM_APP_SIZE_DISABLE=true hence skipping file size checking and going to use found file as application source`);
                            newApp = localFile;
                            downloadIsNeeded = false;
                        }
                    }
                    else if (await support_1.fs.exists(lockFile)) {
                        logger_1.default.info(`[MCLOUD] Local version of app not found but .lock file exists. Waiting for .lock to disappear`);
                        logger_1.default.info(`[MCLOUD] .lock file parameters. Created time: ${(await support_1.fs.stat(lockFile)).ctime.toLocaleString('en-US', { timeZoneName: 'short' })}; device/session details: ${await support_1.fs.readFile(lockFile, 'utf8')}`);
                        // Wait for some time till App is downloaded by some parallel Appium instance
                        let attemptsCount = 0;
                        while (await support_1.fs.exists(lockFile) && (attemptsCount++ < maxAttemptsCount)) {
                            const stats = await support_1.fs.stat(lockFile);
                            let diffInSeconds = (new Date().getTime() - stats.ctime.getTime()) / 1000;
                            if (diffInSeconds >= maxLockFileLifetime) {
                                logger_1.default.info(`[MCLOUD] Removing .lock file since its lifetime reached to the limit`);
                                if (await support_1.fs.exists(lockFile)) {
                                    await support_1.fs.unlink(lockFile);
                                    ...
                                    logger_1.default.warn(`[MCLOUD] Lock file ${lockFile} was not found. Probably it was removed by another thread which was downloading app in parallel`);
                                }
                            }
                            await new Promise((resolve) => {
                                logger_1.default.info(`[MCLOUD] Attempt #${attemptsCount} for .lock file checking`);
                                setTimeout(resolve, waitingTime);
                            });
                        }
                        if (await support_1.fs.exists(lockFile)) {
                        ...
                        logger_1.default.info(`[MCLOUD] Local version of app was found after .lock file removal. Will use it for new session`);
                        newApp = localFile;
                        downloadIsNeeded = false;
                    }
                    else {
                        logger_1.default.info(`[MCLOUD] Neither local version of app nor .lock file was found. Will download app from remote URL.`);
                        downloadIsNeeded = true;
                    }
                }
                else {
                    logger_1.default.info(`[MCLOUD] Local apps folder is not defined via environment properties, hence skipping this logic. Use variable APPIUM_APPS_DIR for path setting`);
                }
                if (downloadIsNeeded) {
                    if (localAppsFolder !== undefined) {
                        logger_1.default.info(`[MCLOUD] Local version of app was not found. Hence using default Appium logic for downloading`);
                        const sharedFolderPath = await (0, mcloud_utils_1.getSharedFolderForAppUrl)(app);
                        logger_1.default.info(`[MCLOUD] Folder for local shared apps: ${sharedFolderPath}`);
                        // @ts-ignore
                        await support_1.fs.close(await support_1.fs.open(lockFile, 'w'));
                        ...
                        logger_1.default.info(`[MCLOUD] session/device info: ${msg}`);
                        await support_1.fs.writeFile(lockFile, msg);
                        logger_1.default.info(`[MCLOUD] lock file was written successfully`);
                    }
                    try {
                        if (cachedAppInfo && status === HTTP_STATUS_NOT_MODIFIED) {
                            if (await isAppIntegrityOk(/** @type {string} */ (cachedAppInfo.fullPath), cachedAppInfo.integrity)) {
                                logger_1.default.info(`[MCLOUD] Reusing previously downloaded application at '${cachedAppInfo.fullPath}'`);
                                return verifyAppExtension(/** @type {string} */ (cachedAppInfo.fullPath), supportedAppExtensions);
                            }
                            logger_1.default.info(`[MCLOUD] The application at '${cachedAppInfo.fullPath}' does not exist anymore ` +
                        ...
                        for (let attemptsLeft = appFetchRetries; attemptsLeft >= 0; attemptsLeft--) {
                            ...
                            try {
                                logger_1.default.info(`[MCLOUD] Retry fetching remote app, attempts left ${attemptsLeft}`);
                                ...
                                if (onDownload) {
                                    newApp = await onDownload({
                                        url: originalAppLink,
                                        headers: /** @type {import('@appium/types').HTTPHeaders} */ (lodash_1.default.clone(attemptHeaders)),
                                        stream: attemptStream,
                                    });
                                    // If onDownload returns a local path then validate it exists.
                                    if (!newApp || !(await support_1.fs.exists(newApp))) {
                                    ...
                                    newApp = await fetchApp(attemptStream, await support_1.tempDir.path({
                                        prefix: fileName,
                                        suffix: '',
                                    }));
                                }
                                ...
                                logger_1.default.error(`[MCLOUD] Error during fetching of the application ${err.message}. Attempts left ${attemptsLeft - 1}`);
                            }
                            finally {
                        ...
                        if (localAppsFolder !== undefined) {
                            logger_1.default.info(`[MCLOUD] New app path: ${newApp}`);
                            // @ts-ignore
                            await support_1.fs.copyFile(newApp, localFile);
                        }
                    }
                    finally {
                        if (localAppsFolder !== undefined) {
                            logger_1.default.info(`[MCLOUD] Going to remove lock file ${lockFile}`);
                            // @ts-ignore
                            if (await support_1.fs.exists(lockFile)) {
                                // @ts-ignore
                                await support_1.fs.unlink(lockFile);
        ...
        const storeAppInCache = async (appPathToCache) => {
            ...
            if (cachedFullPath && cachedFullPath !== appPathToCache && localAppsFolder === undefined) {
                logger_1.default.info(`[MCLOUD] storeAppInCache(). Deleting files by path: ${cachedFullPath}`);
