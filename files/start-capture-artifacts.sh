...
ffmpeg -f mjpeg -r 10 -i tcp://${BROADCAST_HOST}:${BROADCAST_PORT} -vf scale="-2:720" -vcodec libx264 -movflags +frag_keyframe+separate_moof+omit_tfhd_offset+empty_moov -y ${FFMPEG_OPTS} /tmp/${sessionId}.mp4 > ${VIDEO_LOG} 2>&1 &
