            ...
            try {
                ...
                if (downloadIsNeeded) {
                    if (localAppsFolder !== undefined) {
                        ...
                        // @ts-ignore - using fs.open/close from @appium/support which does not match TypeScript lib defs
                        await support_1.fs.close(await support_1.fs.open(lockFile, 'w'));
                        let sessionId = process.env.sessionId;
                        let deviceId = process.env.DEVICE_UDID;
                        ...
                        let msg = `device UUID: ${deviceId}, session ID: ${sessionId}`;
                    ...
                    try {
                        ...
                        while (appFetchRetries-- >= 0 && !(await support_1.fs.exists(newApp))) {
                            try {
                                logger_1.default.info(`[MCLOUD] Going to fetch remote app`);
                                newApp = onDownload
                                    ? await onDownload({
                                        url: originalAppLink,
                                ...
                                logger_1.default.error(`[MCLOUD] Error during fetching of the application ${err.message}. Attempts left ${appFetchRetries}`);
                            }
                        }
                        // ***** Custom logic for copying of downloaded app to static location *****
                        if (localAppsFolder !== undefined) {
                            logger_1.default.info(`[MCLOUD] New app path: ${newApp}`);
                            // @ts-ignore - fs.copyFile exists on the runtime fs wrapper though its TS signature may differ
                        ...
                        if (localAppsFolder !== undefined) {
                            ...
                            // @ts-ignore - fs.exists is provided by the @appium/support fs wrapper and may not match TS types
                            if (await support_1.fs.exists(lockFile)) {
                                // @ts-ignore - fs.unlink is available at runtime though its TS signature may differ
                                await support_1.fs.unlink(lockFile);
                            }
                            else {
                                logger_1.default.warn(`[MCLOUD] Lock file ${lockFile} was not found. Probably it was removed by another thread which was downloading app in parallel`);
        ...
        if (isPackageAFile && shouldUnzipApp && !lodash_1.default.isFunction(onPostProcess)) {
            ...
            if (packageHash === cachedAppInfo?.packageHash) {
                ...
                if (await isAppIntegrityOk(/** @type {string} */ (fullPath), cachedAppInfo?.integrity)) {
                    if (archivePath !== app && localAppsFolder === undefined) {
                        logger_1.default.info(`[MCLOUD] isAppIntegrityOk=true. Deleting files by path: ${archivePath}`);
            ...
            try {
                ...
                if (newApp !== archivePath && archivePath !== app && localAppsFolder === undefined) {
                    logger_1.default.info(`[MCLOUD] Remove after unzipApp(). Deleting files by path: ${archivePath}`);
                    await support_1.fs.rimraf(archivePath);
                }
            }
...
async function unzipApp(zipPath, dstRoot, supportedAppExtensions) {
    ...
    try {
        ...
        logger_1.default.info(`[MCLOUD] unzipApp(). Deleting files by path: ${tmpRoot}`);
