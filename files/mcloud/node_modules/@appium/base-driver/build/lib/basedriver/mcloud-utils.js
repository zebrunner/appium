...
async function getLocalFileForAppUrl(url) {
    ...
    const targetPath = path.join(await getLocalAppsFolder(), sub);
    ...
    const getLengthRequest = async () => {
        for (let i = 0; i < retries; i++) {
            try {
                ...
                lastError = error;
                logger_1.default.warn(`[MCLOUD] Cannot fetch info about app size. Will retry attempt in ${pollingInterval}ms`);
...
async function getWDAStatus(wdaURL) {
    try {
        ...
        logger_1.default.info(`[MCLOUD] Cannot send GET request to '${wdaURL}'. Original error: ${e.message}`);
        return undefined;
    }
}
