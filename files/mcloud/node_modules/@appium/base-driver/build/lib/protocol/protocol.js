...
function buildHandler(app, method, path, spec, driver, isSessCmd) {
    let asyncHandler = async (req, res) => {
        ...
        try {
            ...
            const token = process.env.STF_AUTH_TOKEN
            ...
            if (err.message.toLowerCase().includes('no space left on device') || err.message.toLowerCase().includes('adb: failed to install')) {
                ...
                updateDevice(token, udid, err.message, 'Unhealthy') // device status will change to Unhealthy
                    ...
                    .catch(error => {
                        console.log(`[MCLOUD] Error occurred during making device with UDID ${udid} Unhealthy. Error: ${error}.`)
                    });
            }
            ...
            if (err.message.toLowerCase().includes("socket hang up") && err.message.toLowerCase().includes("could not proxy command to the remote server")) {
                ...
                updateDevice(token, udid, err.message)  // device status will not change
                    ...
                    .catch(error => {
                        console.log(`[MCLOUD] Error occurred while adding comment for device with UDID in STF ${udid}. Error: ${error}`)
                    });
            }
