...
+import path from 'path';
 import {
   ...
   type AppiumServer,
@@ -24,6 +26,7 @@ import {DELETE_SESSION_COMMAND, determineProtocol, errors} from '../protocol';
...
+        });
+      } catch (err) {
+        this.log.info(`Error creating the .recording-artifact-${sessionId} file: ${err}`);
+      }
+    };
+    
+    // directory where .recording-artifact file should be created
-    const logDir = process.env.LOG_DIR || './'
-    await createRecordingArtifact(this.sessionId, logDir)
...
@@ -349,6 +376,22 @@ export class BaseDriver<
         queues[key] = [];
       }
     }
...
-    const fileName = path.join(dirName, `.recording-artifact-${this.sessionId}`);
...
+            async function updateDeviceStatus(token, udid, note) {
+
...
+              }
+              
+              const token = process.env.STF_TOKEN || null
+              const udid = process.env.DEVICE_UDID || null
+              
+
...
+            console.log(`[MCLOUD] Cannot find any free port in range. Error message: ${err.message} Appium exit with code 1 `);
...
-                console.log(`- [BaseDriver] ENOSPC error, no space left on device, Error message:${err.message} Appium need restart, exit code 0`)
...
+                 // process.exit(0);
+                updateDeviceStatus(token, udid, err.message)
+                .then(() => {
...
-                console.log(`- [BaseDriver] Error occurred, socket hang up. Original error: ${err.message} Appium need restart, exit code 0`);
+                console.log(`[MCLOUD] [BaseDriver] Error occurred, socket hang up. Original error: ${err.message}. exit code 0. Device is Unhealthy !!!`);
+                 // process.exit(0);
+                updateDeviceStatus(token, udid, err.message)
+                .then(() => {
+                  console.log(`[MCLOUD] Devices status for device with UDID ${udid} changed to Unhealthy!`)
...
-                console.log(`- [BaseDriver] Error occurred, device setup is not yet complete. Original error: ${err.message} Appium need restart, exit code 0`);
...
+                console.log(`[MCLOUD] [BaseDriver] Got error: the session identified by [[[--udid ${process.env.DEVICE_UDID} --name ${process.env.DEVICE_NAME} --sessionId ${process.env.sessionId}]]] is not known. Error message: ${err.message}. Exit code 0. Device is Unhealthy !!!`);
+                // process.exit(0);
+                updateDeviceStatus(token, udid, err.message)
+                .then(() => {
+                  console.log(`[MCLOUD] Devices status for device with UDID ${udid} changed to Unhealthy!`)
