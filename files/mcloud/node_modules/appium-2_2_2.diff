...
+                };
+              
+                const data = status ? { device: { status: status, note: note } } : { device: { note: note } }
+
+                try {
...
+                 // process.exit(0);
+                updateDevice(token, udid, err.message, 'Unhealthy') // device status will change to Unhealthy
+                .then(() => {
...
+                // process.exit(0);
+                updateDevice(token, udid, err.message) // device status will not change
+                .then(() => {
+                  console.log(`[MCLOUD] Devices status for device with UDID ${udid} changed to Unhealthy!`)
+                })
+                .catch(error => {
