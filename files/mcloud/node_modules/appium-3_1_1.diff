...
@@ -209,6 +210,58 @@ export class DriverCore<const C extends Constraints, Settings extends StringReco
     return {};
   }
...
index 07258f35b..7f1373a2f 100644
...
+import path from 'path';
 import {
   ...
   type AppiumServer,
@@ -360,6 +362,30 @@ export class BaseDriver<
...
+        this.log.info(`.recording-artifact-${sessionId} file created successfully.`);
+      } catch (err) {
+        this.log.error(`Error creating the .recording-artifact-${sessionId} file, ${err}`);
+      }
+    };
...
@@ -392,6 +418,27 @@ export class BaseDriver<
         queues[key] = [];
       }
     }
+
...
+        this.log.info(`File ${fileName} removed successfully.`);
+      } catch (err) {
+        this.log.error(`Error removing file ${fileName}: ${err}`);
+      }
+    };
+    const logDirName: string = process.env.LOG_DIR ?? '/tmp/log';
+    // Check if process.env.ROUTER_UUID is not empty
...
index b3bdf5a2e..09429beae 100644
...
@@ -28,6 +29,7 @@ const APPLICATIONS_CACHE = new LRUCache({
...
@@ -100,6 +103,7 @@ export async function configureApp(
   let newApp = app;
...
+  let localAppsFolder;
   ...
   let headers;
   /** @type {RemoteAppProps} */
@@ -161,38 +165,201 @@ export async function configureApp(
             }
           }
         }
...
+                    throw new Error(`onDownload did not return a valid existing app path`);
+                  }
+                } else {
+                  if (!fileName) {
...
@@ -218,7 +385,10 @@ export async function configureApp(
...
index e69de29bb..1fd874377 100644
--- a/packages/base-driver/lib/basedriver/mcloud-utils.ts
...
+  let sub = url.substring(url.indexOf('//') + 2);
...
+
+  const targetPath = path.join(await getLocalAppsFolder() as string, sub);
...
+      } catch (error) {
+        lastError = error;
+        logger.warn(`[MCLOUD] Cannot fetch info about app size. Will retry attempt in ${pollingInterval}ms`);
...
+}
+
+async function executeShell(shellCommand, description) {
+  const execPromisify = util.promisify(exec);
+  try {
+    const { stderr } = await execPromisify(shellCommand);
+    if (stderr) {
...
+  getWDAStatus
+};
diff --git a/packages/base-driver/lib/protocol/protocol.ts b/packages/base-driver/lib/protocol/protocol.ts
index 02822a28c..67850031c 100644
--- a/packages/base-driver/lib/protocol/protocol.ts
+++ b/packages/base-driver/lib/protocol/protocol.ts
@@ -474,6 +474,115 @@ function buildHandler(
           })}`
       );
     } catch (err) {
...
+          getLogger(driver, req.params.sessionId || newSessionId).info('Successful response: ', responseData);
+
+          return responseData;
+        } catch (error) {
+          getLogger(driver, req.params.sessionId || newSessionId).error('Request error: ', error.message);
+          throw error;
+        }
+      }
+
...
+  },
   '/session/:sessionId/timeouts': {
     ...
     POST: {
@@ -637,4 +643,4 @@ function toCommandNameCacheKey(endpoint, method) {
 }
...
+export const NO_SESSION_ID_COMMANDS = ['createSession', 'getStatus', 'getAppiumSessions', 'getStatusWDA', 'getStatusADB'];
