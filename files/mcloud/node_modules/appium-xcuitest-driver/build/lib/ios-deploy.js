...
class IOSDeploy {
    ...
    async remove(bundleId) {
        try {
            await (0, teen_process_1.exec)(IOS_DEPLOY, [
                'uninstall', bundleId,
                '--udid=' + this.udid
            ]);
        }
    ...
    async isAppInstalled(bundleId) {
        ...
        try {
            let { stdout, stderr, code } = await (0, teen_process_1.exec)(IOS_DEPLOY, [
                'apps',
                '--system',
                '--udid=' + this.udid
            ]);
            if (stdout != null && stdout.indexOf(bundleId) !== -1) {
                logger_1.default.debug(bundleId + ' is found among system apps.');
                return true;
            }
            else {
                logger_1.default.debug(bundleId + ' is NOT found among system apps.');
            }
        }
        catch (err1) {
        ...
        try {
            ...
            if (stdout != null && stdout.indexOf(bundleId) !== -1) {
                logger_1.default.debug(bundleId + ' is found among non system apps.');
                return true;
            }
            else {
                logger_1.default.debug(bundleId + ' is NOT found among non system apps.');
            }
        }
        catch (err1) {
