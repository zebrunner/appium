...
index 3f088c49f..b244db21e 100644
...
@@ -207,6 +208,60 @@ export class DriverCore<const C extends Constraints, Settings extends StringReco
     return {};
   }
...
index 21e48c08f..b26d5b579 100644
...
@@ -94,6 +99,7 @@ export async function configureApp(
...
+  let localAppsFolder;
   ...
   /** @type {RemoteAppProps} */
@@ -155,6 +161,128 @@ export async function configureApp(
             }
           }
         }
+
...
@@ -223,16 +351,47 @@ export async function configureApp(
           }
...
+          }
         ...
         if (!stream.closed) {
           stream.destroy();
@@ -263,7 +422,8 @@ export async function configureApp(
...
@@ -279,7 +439,8 @@ export async function configureApp(
       try {
...
@@ -288,7 +449,10 @@ export async function configureApp(
...
+        logger.info(`[MCLOUD] storeAppInCache(). Deleting files by path: ${cachedFullPath}`);
       ...
       const integrity = {};
@@ -565,6 +729,7 @@ async function unzipApp(zipPath, dstRoot, supportedAppExtensions) {
...
index 7ef8df34b..8b2e27c41 100644
--- a/packages/base-driver/lib/protocol/protocol.js
+++ b/packages/base-driver/lib/protocol/protocol.js
@@ -454,6 +454,9 @@ function buildHandler(app, method, path, spec, driver, isSessCmd) {
           })}`
       );
     } catch (err) {
...
@@ -1008,4 +1014,4 @@ function toCommandNameCacheKey(endpoint, method) {
 }
...
+export const NO_SESSION_ID_COMMANDS = ['createSession', 'getStatus', 'getStatusWDA', 'getStatusADB', 'getSessions', 'getAppiumSessions'];
