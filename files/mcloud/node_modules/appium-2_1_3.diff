...
index 63f480b1e..dc076426a 100644
...
+  let localAppsFolder;
   ...
   /** @type {RemoteAppProps} */
@@ -178,6 +184,123 @@ async function configureApp(
             }
           }
         }
+
...
@@ -250,7 +373,37 @@ async function configureApp(
           prefix: fileName,
           suffix: '',
         });
...
+        }
         ...
         if (!stream.closed) {
           stream.destroy();
@@ -281,7 +434,8 @@ async function configureApp(
...
@@ -297,7 +451,8 @@ async function configureApp(
       try {
...
@@ -314,6 +469,7 @@ async function configureApp(
...
+        logger.info(`[MCLOUD] storeAppInCache(). Deleting files by path: ${cachedFullPath}`);
       ...
       const integrity = {};
@@ -509,6 +665,7 @@ async function unzipApp(zipPath, dstRoot, supportedAppExtensions) {
