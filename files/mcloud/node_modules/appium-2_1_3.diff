...
index 63f480b1e..d07354ef2 100644
...
@@ -30,6 +33,7 @@ const APPLICATIONS_CACHE = new LRU({
...
@@ -128,6 +133,7 @@ async function configureApp(
   let newApp = app;
...
+  let localAppsFolder;
   ...
   /** @type {RemoteAppProps} */
@@ -178,6 +184,126 @@ async function configureApp(
             }
           }
         }
+
...
@@ -250,7 +376,37 @@ async function configureApp(
           prefix: fileName,
           suffix: '',
         });
...
+        }
         ...
         if (!stream.closed) {
           stream.destroy();
@@ -281,7 +437,8 @@ async function configureApp(
...
@@ -297,7 +454,8 @@ async function configureApp(
       try {
...
@@ -314,6 +472,7 @@ async function configureApp(
...
+        logger.info(`[MCLOUD] storeAppInCache(). Deleting files by path: ${cachedFullPath}`);
       ...
       const integrity = {};
@@ -509,6 +668,7 @@ async function unzipApp(zipPath, dstRoot, supportedAppExtensions) {
