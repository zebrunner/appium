  ...
  async executeCommand (cmd, ...args) {
    ...
    if (cmd === 'createSession') {
      ...
      if(res != undefined && res.value != undefined) {
        log.info("stopping fallback session recording");
        // stopping and removing fallback recording
        const stop_rec_command = `sh /opt/stop-capture-artifacts.sh`;
        executeShell(stop_rec_command, 'stop video recording');
        await new Promise(resolve => setTimeout(resolve, 300));
        // starting new session recording
        log.info("starting new video recording on session init")
        const start_rec_command = `sh /opt/capture-artifacts.sh ${res.value[0]}`;
        executeShell(start_rec_command, 'start video recording');
      }


      // let sessionId = res.value[0];
      // // const videoFile="AAA";
      // const part=0;
      // const SCREENRECORD_OPTS = '';
      // const start_rec_command = `adb shell "screenrecord --verbose ${SCREENRECORD_OPTS} /sdcard/${sessionId}_${part}.mp4"`;

      // log.info(`CUSTOM4!! session id ${res.value[0]}`);
      // // console.log(res);
      // log.info('starting video recording for the session')
      // exec(start_rec_command, (error, stdout, stderr) => {
      //   if (error) {
      //       log.info(`start video recording error: ${error.message}`);
      //       // return;
      //   }
      //   if (stderr) {
      //       log.info(`start video recording stderr: ${stderr}`);
      //       // return;
      //   }
      //   log.info('video recording was successfully started');
      // });
    } else if (cmd === 'deleteSession') {
